...
pub fn euler26_example() {
    println!("\nProblem: Find the value of d < 1000 for which 1/d contains the longest recurring cycle in its decimal fraction part.");
    println!("\n\nLong division is the main thing needed to solve this. Numbers the factor entirely into 2s and 5s could be discarded for having no cycle but that's wouldn't improve speed at all.");
    let s = "
fn decimal_part_len(n: u64) -> u64 {
    let mut digits = Vec::new();
    let mut partials = Vec::new();
